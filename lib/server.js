...
class ChromeServer {
  ...
  constructor(options) {
    ...
    app.use(route.get('/api/version', function (ctx) {
      ...
      return findChromeVersion(ctx.request.query).then((data) => {
        ctx.body = {
          data: {
            version: require('../package.json').version,
            startup: startup.toISOString(),
    ...
    app.use(route.get('/api/proxy/:ip', function (ctx, ip) {
      ...
      return launchChrome(id, { address: '127.0.0.1' }, { chromeFlags }, query).then(chrome => {
        return evalInChrome(chrome, url, expression).then(rsp => {
